                ...
                for data_stddev in $global_data_stddevs; do
                    for spatial_stddev in $global_spatial_stddevs; do
                        ...
                        if [ $iter -eq 1 ]; then
                            ...
                            local input_volume_name="$(get_volume_name $((iter-1)) $patch_size $pca_dim $data_stddev $spatial_stddev).am"
                        fi
                        ...
                        local output_volume_name="$(get_volume_name $iter $patch_size $pca_dim $data_stddev $spatial_stddev).am"
